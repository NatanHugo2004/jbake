...
public abstract class MarkupEngine implements ParserEngine {
  ...
  private boolean isUtf8WithBOM(String encoding, String line) {
    return "UTF-8".equals(encoding) && line.startsWith(UTF_8_BOM);
  }
  ...
  void storeHeaderValue(String inputKey, String inputValue, DocumentModel content) {
    ...
    if (ModelAttributes.DATE.equalsIgnoreCase(key)) {
    ...
    } else if (ModelAttributes.TAGS.equalsIgnoreCase(key)) {
      content.setTags(getTags(value));
    } else if (ModelAttributes.CACHED.equalsIgnoreCase(key)) {
