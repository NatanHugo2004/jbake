...
public class FileUtil {
  ...
  public static boolean directoryOnlyIfNotIgnored(File file, JBakeConfiguration config) {
    if (file == null || config == null || !file.isDirectory()) {
      return false;
    }
    final String ignoreFileName = config.getIgnoreFileName();
    FilenameFilter ignoreFile = new FilenameFilter() {
      @Override
      public boolean accept(File dir, String name) {
        return name.equalsIgnoreCase(ignoreFileName); // Usa a variável local isolada
      }
    };
    File[] matchedFiles = file.listFiles(ignoreFile);
  ...
  public static boolean directoryOnlyIfNotIgnored(File file) {
    ...
    FilenameFilter ignoreFile = new FilenameFilter() {
      @Override
      public boolean accept(File dir, String name) {
        return ".jbakeignore".equalsIgnoreCase(name);
      }
    };
    File[] matchedFiles = file.listFiles(ignoreFile);
    return matchedFiles != null && matchedFiles.length == 0;
  ...
  public static String asPath(String path) {
    ...
    if (!FileUtil.URI_SEPARATOR_CHAR.equals(File.separator)) {
